...
- (BOOL)isActiveBusinessHours
{
  ...
  NSDate *nowUnOffset = [NSDate date];
  
  //convert to GlobalTime
  NSTimeZone *tz = [NSTimeZone defaultTimeZone];
  NSInteger seconds = -[tz secondsFromGMTForDate: nowUnOffset];
  NSDate * globalNowTime = [NSDate dateWithTimeInterval: seconds sinceDate: nowUnOffset];
  
  //convert global time to local time of business schedule
  NSTimeZone* bHoursTimeZone = [[NSTimeZone alloc] initWithName:businessHours.timezone];
  NSInteger secFromGmt = [bHoursTimeZone secondsFromGMT];
  NSInteger hoursFromGmt = [bHoursTimeZone secondsFromGMT] / 60 / 60;
  NSDate *now = [globalNowTime dateByAddingTimeInterval:3600*hoursFromGmt];
  
  //to use previous version's date logic, use this definition of now:
